...
class ApplicationInfo extends Model implements HasMedia
{
    ...
    public function getBrandLogoDarkAttribute()
    {
        if ($this->getFirstMedia('logo-dark') != null) {
            if ($this->getFirstMedia('logo-dark')->hasGeneratedConversion('medium')) {
                $fullPath = $this->getFirstMedia('logo-dark')->getPath('thumb');
                if (file_exists($fullPath)) {
                    return $this->getFirstMedia('logo-dark')->getUrl('thumb');
                }
            } else {
                $fullPath = $this->getFirstMedia('logo-dark')->getPath();
                if (file_exists($fullPath)) {
                    return $this->getFirstMedia('logo-dark')->getUrl();
                }
            }
        }
