<template>
  <div class="el-form el-form--default el-form--label-top ">
    <el-row :gutter="10">
      <el-col :span="5">
        <el-form-item label="">
          <el-select
              ...
              filterable
          >
            <el-option v-for="(product, key) in products" :disabled="product.available_quantity == 0" :key="key"
                       :label="product.name" :value="product" :value-key="product.name">
      ...
      <el-col :span="3">
        <el-form-item label="">
          <el-input-number v-model="quantity" @keydown.enter.shift.prevent="handleShiftEnter" :min="1"
                           :max="maxQuantity" @keyup="updatePrice" @change="updatePrice"/>
        </el-form-item>
      </el-col>
      <el-col :span="3">
        <el-form-item label="">
      ...
      <el-col :span="1">
        <el-button type="danger" @keydown.enter.shift.prevent="handleShiftEnter" @click="removeProduct" size="small"
                   :icon="Close" circle/>
      </el-col>
    </el-row>
  </div>
...
const selectProduct = (productInfo) => {
  selectedProduct.value = productInfo.name
  ...
  rate.value = productInfo.current_discount == null ? comboOffers.value['price_1'] : (comboOffers.value['price_1'] - (+productInfo.current_discount.discount / 100) * comboOffers.value['price_1']);
